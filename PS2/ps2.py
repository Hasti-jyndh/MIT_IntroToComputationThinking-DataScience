...
def load_map(map_filename):
    ...
    # TODO
    # source destination total_time outdoor_time
    diagrapgh = Digraph()
    print("Loading map from file...")
    with open(map_filename) as mit_map:
        lines = mit_map.readlines()
        for line in lines:
            splited=line.split(' ')
            src = Node(splited[0])
            dest = Node(splited[1])
            if not diagrapgh.has_node(src):
                diagrapgh.add_node(src)
            if not diagrapgh.has_node(dest):
                diagrapgh.add_node(dest)
            diagrapgh.add_edge(WeightedEdge(src, dest, int(splited[2]), int(splited[3].rstrip())))
    return diagrapgh
